...
socket_calculate_checksum(const uint8_t *buffer, size_t buffer_len, uint16_t *checksum)
{
    ...
    uint32_t sum = 0;
    const uint16_t *short_buffer = NULL;
    size_t short_buffer_len = 0;
    size_t i = 0;
    ...
    /* Get access to buffer as array of short */
    short_buffer = (uint16_t *)buffer;
    short_buffer_len = buffer_len / sizeof(*short_buffer);
                                         ...
                                         uint16_t *checksum)
{
    ...
    packet_len = (sizeof(checksum_header) + data_len);

    /* Allocate memory for checksum packet */
    checksum_packet = HEAPALLOCZ(sizeof(*checksum_packet) * packet_len);
    ...
    /* Calculate Checksum */
    status = socket_calculate_checksum(checksum_packet, packet_len, &temp_checksum);
...
socket_get_syn_filter(int16_t port, struct sock_filter **filter_instructions, size_t *filter_length)
{
    ...
    temp_filter_length = ARRAY_LEN(socket_filter_instructions);

    /* Allocate memory for filter instructions */
    temp_filter = HEAPALLOCZ(sizeof(*temp_filter) * temp_filter_length);
    if (NULL == temp_filter) {
    ...
    for (i = 0; i < temp_filter_length; ++i) {
        temp_filter[i] = socket_filter_instructions[i];
    }
    ...
    /* transfer Ownership */
    *filter_length = temp_filter_length;
                                    ...
                                    char *ip)
{
    ...
    /* Transfer Ownership */
    *data_len = packet_len - (ip_header->ihl * BYTES_IN_WORD) - (tcp_header->doff * BYTES_IN_WORD);
...
enum zash_status SOCKET_tcp_server(uint16_t port, int *socket_fd)
{
    ...
    *socket_fd = server_socket;
    server_socket = INVALID_FILE_DESCRIPTOR;
...
enum zash_status SOCKET_accept(int server_socket, int *socket_fd)
{
    ...
    /* Set the linger timeout so the bound port will be unbind sa soon as the socket closed */
...
enum zash_status SOCKET_tcp_client(const char *ip, uint16_t port, int *socket_fd)
{
